...
declare
Cursor LockCur is SELECT l.SID SID,
...
and l.sid = s.sid
and lo.ORACLE_USERNAME='MTS24E'
and upper(s.machine) not like '%MICHAEL%'
and upper(s.machine) not like '%ZEUS%'
...
begin
    ...
    for j in 0 .. LockTab.count-1 loop
        dbms_output.put_line('Alter system kill session '''||LockTab(j).sid||','||
	LockTab(j).ser#||''';'||'/* '||substr(LockTab(j).machine,1,length(LockTab(j).machine)-1)||
	' '||LockTab(j).orauser||'*/');
    end loop;
end;
/
