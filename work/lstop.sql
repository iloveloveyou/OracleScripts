...
prompt Incremental LATCH REQUESTS BY SID
...
-- Сбор статистики по латчам
    ...
    for i in 0..n_i-1 loop
        if arr2.exists(arr1(i).sid) then
           arr2(arr1(i).sid).cnt :=  arr2(arr1(i).sid).cnt +1;
        else
            arr2(arr1(i).sid).cnt := 1;
            arr2(arr1(i).sid).sid := arr1(i).sid;
        end if;
    end loop;
   ...
   --------------------------------------------------------
    ...
    dbms_output.put_line('SID         REQUESTS');
    dbms_output.put_line('--------------------');

    for i in 1..least(ShowRows#,max_j-1) loop
        dbms_output.put_line(rpad(arr3(i).sid, 10) || ' ' ||
