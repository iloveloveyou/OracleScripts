...
run {
	...
	backup full database format '$HOTBACKUP_PATH/%d_%I_$BACKUP_FILE_PREFIX\_datafile_%s_%p_%D.%M.%Y.bak' 
		noexclude include current controlfile tag 'PRIMARY_HOTBACKUP_DB' 
			plus archivelog format '$HOTBACKUP_PATH/%d_%I_%e_$BACKUP_FILE_PREFIX\_archlogs_%s_%p_%D.%M.%Y.bak' tag 'PRIMARY_HOTBACKUP_ARCH' delete input;
	backup current controlfile for standby format '$HOTBACKUP_PATH/%d_%I_standby_controlfile_%s_%p_%D.%M.%Y.bak' tag 'PRIMARY_HOTBACKUP_STNDB_CTL'; 
}
host '
echo "#################################################" >> $HOTBACKUP_LOG;
